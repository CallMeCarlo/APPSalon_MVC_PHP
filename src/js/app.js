...
function iniciarApp() {
    ...
    tabs(); //Cambia la sección cuando se seleccionan los tabs
...
function mostrarSeccion() {
    ...
    //Ocultar la que tenga clase de mostrar
...
function seleccionarHora() {
    ...
    inputHora.addEventListener("input", function(e) {
        ...
        if(hora < 10 || hora > 18) {
            e.target.value = "";
            mostrarAlerta("El horario es de 10 a 6 de la tarde", "error", ".formulario");
        } else {
...
function mostrarAlerta(mensaje, tipo, elemento, desaparece = true) {
    ...
    //Previenen que se generen más de una alerta
...
function mostrarResumen() {
    ...
    if(Object.values(cita).includes("") || cita.servicios.length === 0) {
        mostrarAlerta("Faltan datos o servicios en la cita", "error", ".contenido-resumen", false);
    ...
    //Heading para datos del cliente en resumen
    const headingDatos = document.createElement("H3");
    headingDatos.textContent = "Datos del cliente";
...
async function reservarCita() {
    ...
    if(resultado.resultado) {
        Swal.fire({
            icon: 'success',
            title: 'Cita Creada',
            text: 'Tu cita fue creada correctamente :D',
            button: 'OK'
          }).then( () => {
                setTimeout(() => {
        ...
        Swal.fire({
            icon: 'error',
            title: 'Oops...',
            text: 'Hubo un error al crear tu cita intentalo nuevamente!'
          })
    }
